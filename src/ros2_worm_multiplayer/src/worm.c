...
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>  // bools can now be used
#include <time.h>
#include <string.h>
#include <unistd.h>
        ...
        switch(ch) {
            case 'q' :  // User wants to end the show
                *agame_state = WORM_GAME_QUIT;
                break;
            case 'g' :  // User wants to grow the worm
...
enum ResCodes doLevel() {
    ...
    switch (game_state) {
      case WORM_GAME_ONGOING:
        if (getNumberOfFoodItems(&theboard) == 0) {
          showDialog("Sie haben diese Runde gewonnen!", "Bitte Taste drücken.  ");
        } else {
      ...
      case WORM_OUT_OF_BOUNDS:
        showDialog("Sie habe das Spiel verloren, weil Sie das Spielfeld verlassen haben!", "Bitte Taste drücken.  ");
        break;
      case WORM_CROSSING:
        showDialog("Sie haben das Spiel verloren, weil Sie einen Wurm gekreuzt haben!", "Bitte Taste drücken.  ");
        break;
      case WORM_CRASH:
        showDialog("Sie haben das Spiel verloren, weil Sie gegen eine Barriere gekommen sind!", "Bitte Taste drücken.  ");
        break;
      default:
        showDialog("Interner Fehler!", "Bitte Taste drücken.  ");
